...
from scapy.all import RadioTap, Dot11, Dot11Deauth, Dot11Beacon, Dot11ProbeResp, Dot11Elt
...
def get_iface(interfaces):
    ...
    for iface in interfaces:
        ...
        for line in proc.communicate()[0].split(b'\n'):
            if b' - Address:' in line: # first line in iwlist scan for a new AP
               count += 1
...
def mon_mac(mon_iface):
   ...
   '''
    ...
    mac = ':'.join(['{:02x}'.format(char) for char in info[18:24]])
...
def channel_hop(mon_iface, args):
   ...
   '''
    ...
    while 1:
        ...
        err = None
        for line in proc.communicate()[1].split(b'\n'):
...
def output(err, ca):
    global W, T, O
    print ('['+T+'*'+W+'] '+O+ca[0].decode()+W+' - '+O+ca[1].decode()+W+' - '+ca[2].decode().ljust(2)+' - '+T+ca[3].decode()+W)
    if err == 1:
        print ('['+T+'!'+W+'] '+R+'WARNING:'+W+' Deauthing '+O+ca[1].decode()+W+' on channel '+T+ca[2].decode()+W+' took too long.')
    elif err == 2:
        print ('['+T+'!'+W+'] '+R+'ERROR:'+W+' Deauthing '+O+ca[1].decode()+W+' on channel '+T+ca[2].decode()+W+' failed.')
